...
public class LinkedList<E extends Comparable<E>> {
	...
	private void removeAll(Node head, E ele) {
		if (first != null && ele.equals(head.data)) {
			first = first.next;
			n--;
			removeAll(first, ele);
		}
	...
	private void duplicateAll(Node head, E ele) {
		if (head != null && head.data.equals(ele)) {
			n++;
			duplicateAll(head.next, ele);
			Node temp = new Node(ele, head.next);
			head.next = temp;
		} else if (head != null && !head.data.equals(ele)) {
			duplicateAll(head.next, ele);
		} else {
			return;
		}
